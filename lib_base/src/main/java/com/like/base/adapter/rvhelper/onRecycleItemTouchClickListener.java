 ...
 * manipulation of item views within the RecyclerView. onRecycleItemTouchClickListener may intercept
 * a touch interaction already in progress even if the onRecycleItemTouchClickListener is already handling that
 * gesture stream itself for the purposes of scrolling.
 *
 * @see RecyclerView.OnItemTouchListener
 */
public abstract class onRecycleItemTouchClickListener implements RecyclerView.OnItemTouchListener {
    private GestureDetectorCompat mGestureDetector;
    private RecyclerView recyclerView;
    private Set<Integer> childClickViewIds;
    private Set<Integer> longClickViewIds;
    protected BaseRecyleAdapter baseQuickAdapter;
    public static String TAG = "onRecycleItemTouchClickListener";
    ...
    public boolean onInterceptTouchEvent(RecyclerView rv, MotionEvent e) {
        ...
        if (recyclerView == null) {
            ...
            this.baseQuickAdapter = (BaseRecyleAdapter) recyclerView.getAdapter();
                ...
                if (childClickViewIds != null && childClickViewIds.size() > 0) {
                    for (Iterator it = childClickViewIds.iterator(); it.hasNext(); ) {
                        ...
                        if (inRangeOfView(childView, e)&&childView.isEnabled()) {
                            onItemChildClick(baseQuickAdapter, childView, vh.getLayoutPosition());
                    ...
                    onItemClick(baseQuickAdapter, pressedView, vh.getLayoutPosition());
                } else {
                    onItemClick(baseQuickAdapter, pressedView, vh.getLayoutPosition());
                }
                resetPressedView(pressedView);
        ...
        public void onLongPress(MotionEvent e) {
            ...
            if (mIsPrepressed && mPressedView != null) {
                ...
                if (!isHeaderOrFooterPosition(vh.getLayoutPosition())) {
                    ...
                    if (!isChildLongClick){
                        onItemLongClick(baseQuickAdapter, mPressedView, vh.getLayoutPosition());
                        mPressedView.setPressed(true);
                        mIsShowPress = true;
                    }
     ...
     */
    public abstract void onItemClick(BaseRecyleAdapter adapter, View view, int position);
     ...
     */
    public abstract void onItemLongClick(BaseRecyleAdapter adapter, View view, int position);

    public abstract void onItemChildClick(BaseRecyleAdapter adapter, View view, int position);

    public abstract void onItemChildLongClick(BaseRecyleAdapter adapter, View view, int position);
